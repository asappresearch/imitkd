...
class DataParallelCriterion(DataParallel):
    ...
    def forward(self, inputs, *targets, **kwargs):
        ...
        outputs = _criterion_parallel_apply(replicas, inputs, targets[0], kwargs)
        # return Reduce.apply(*outputs) / len(outputs)
        # return self.gather(outputs, self.output_device).mean()
        sizes = torch.tensor([len(x) for x in inputs]).to(self.output_device)
        losses = self.gather(outputs, self.output_device)
        print(sizes)
        return torch.sum(sizes * losses) / torch.sum(sizes)
